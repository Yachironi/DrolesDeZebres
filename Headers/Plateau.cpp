...
void Plateau::updateSAUTEUR(int i, int j) {
	...
	for (unsigned int c = 0; c < 4; c++) {
		...
		if (x < cases.size() && x >= 0 && y < cases[0].size() && y >= 0) {
			if ((cases[x][y]->pion != NULL)) {
				TypeAnimal typeAnimal =
						((Animal*) (cases[x][y]->pion))->getType();
				if (cases[x][y]->pion->getIntitulePion() == "Gazelle"
						&& cases[i][j]->getIdSecteur()
								!= cases[x][y]->getIdSecteur()) {
					int* choix = new int[2];
					choix[0] = x;
					choix[1] = y;
	...
	if (choixDeSaute.size() > 0) {
		cout << "Bonus : Choix de saute du crocodile [" << i << " , " << j
				<< "] sur :" << endl;
		cout << "[";
		cout << "  1=[PAS DE SAUTE]  ";
		for (unsigned int k = 0; k < choixDeSaute.size(); k++) {
			cout << "  " << (k + 2) << "=[" << choixDeSaute[k][0] << ","
					<< choixDeSaute[k][1] << "]  ";
		}
		cout << "]" << endl;
		int choix = -1;
		...
		if ((choix) > 1) {
			...
			choix -= 2;
			pionTMP =
					cases[choixDeSaute[choix][0]][choixDeSaute[choix][1]]->pion;
			cases[choixDeSaute[choix][0]][choixDeSaute[choix][1]]->pion =
					cases[i][j]->pion;
