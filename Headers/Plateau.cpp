...
void Plateau::saisirImpalaJonesPosition() {
	...
	int listePionAtester[4][2] = { { i, j - 1 }, { i - 1, j }, { i, j + 1 }, { i
		...
		for (unsigned int c = 0; c < 4; c++) {
			...
			if (x < cases.size() && x >= 0 && y < cases[0].size() && y >= 0) {
				if ((cases[x][y]->pion != NULL)) {
					...
					if (typeAnimal == EFFRAYE&&cases[i][j]->getIdSecteur()!=cases[x][y]->getIdSecteur()) {
						int* choix = new int[2];
						choix[0]=x;
						choix[1]=y;
		...
		if(choixDeSaute.size()>0){
			...
			for(unsigned int k=0;k<choixDeSaute.size();k++){
				cout<<"  "<<(k+1)<<"=["<<choixDeSaute[k][0]<<","<<choixDeSaute[k][1]<<"]  ";
			}
			cout<<"]"<<endl;
			int choix=-1;
			cin>>choix;
			choix--;
			while(choix<0||choix>=choixDeSaute.size()){
				cin>>choix;
			}
			if((choix+1)!=0){
				Pion* pionTMP;
